...
####other input parameters besides those in theta vector (one could change parameters here)#####
...
c1 = 2.2041e-3 #2.9334e-3
       
c2 = 1.0861e-2 #1.4312e-2
...
z_list1 = np.linspace(1e-4, z_s1, 50)
z_list2 = np.linspace(1e-4, z_s2, 50)
...
#####function computing 2-point auto correlation#######
...
#####function computing 2-point cross correlation#######
def correlation_2pt_cross(class_dict, l, z_list):
    
    P_k2D_cross = np.zeros(len(l))
    
    for i in range(len(l)):
        dP_k2D_cross = np.zeros(len(z_list))
        for j in range(len(z_list)):
            dP_k2D_cross[j] = (1/class_dict['H'][j]) * (9./4.) * (class_dict['H0'])**4 * class_dict['Om0']**2 * ((class_dict['DA_s1']-class_dict['DA'][j])/class_dict['DA_s1'])* ((class_dict['DA_s2']-class_dict['DA'][j])/class_dict['DA_s2']) *(1+z_list[j])**2 * class_dict['Pk'][i,j]
            P_k2D_cross[i] = np.trapz(dP_k2D_cross, z_list)
    ...
    f = interpolate.interp1d(ell, P_k2D_cross)
...
def log_likelihood(theta, data, cov):
    ...
    p_z1 = my_class(Nu_Cosmo, z_list1, ell)
    p_z2 = my_class(Nu_Cosmo, z_list2, ell)
    
    cosmo_dict_z1 = {'H': p_z1.Hubble(),
                  'H0': p_z1.Hubble_0(),
                  'Om0': p_z1.Omega_m0(),
                  'DA_s': p_z1.angular_diameter_dist_zs(),
                  'DA': p_z1.angular_diameter_dist(),
                  'Pk': p_z1.Pk_value()}
    
    cosmo_dict_z2 = {'H': p_z2.Hubble(),
                  'H0': p_z2.Hubble_0(),
                  'Om0': p_z2.Omega_m0(),
                  'DA_s': p_z2.angular_diameter_dist_zs(),
                  'DA': p_z2.angular_diameter_dist(),
                  'Pk': p_z2.Pk_value()}
    
    cosmo_dict_z12 = {'H': p_z1.Hubble(),
                  'H0': p_z1.Hubble_0(),
                  'Om0': p_z1.Omega_m0(),
                  'DA_s1': p_z1.angular_diameter_dist_zs(),
                  'DA_s2': p_z2.angular_diameter_dist_zs(),
                  'DA': p_z1.angular_diameter_dist(),
                  'Pk': p_z1.Pk_value()}
    
    
    #combine different components to form a new tomographic data vector#
    model_z1 = correlation_2pt(cosmo_dict_z1, ell, z_list1)
    model_z2 = correlation_2pt(cosmo_dict_z2, ell, z_list2)
    model_z12 = correlation_2pt_cross(cosmo_dict_z12, ell, z_list1)
    model = np.concatenate((model_z1, model_z2, model_z12))
...
data_vec1 = pd.read_csv('correlationfunc/2ptfunc_circpatch/2pt_mnv0.1_om0.3_As2.1_z1.0_relizations9999_circular.csv', sep=' ')
...
data_vec = pd.concat([data_vec1, data_vec2, data_vec3], axis=1)
data_vec = data_vec.iloc[:4999,:]
data_vec = data_vec.to_numpy()
data_vec = np.mean(data_vec, axis=0)

corr2pt1 = pd.read_csv('correlationfunc/2ptfunc_circpatch_wn/2pt_mnv0.1_om0.3_As2.1_z1.0_relizations10000_circular_LSST.csv', sep=' ')
...
corr2pt = pd.concat([corr2pt1, corr2pt2, corr2pt3], axis=1)
...
filename = "MCMC_30walkers_largerprior_z1.0and1.5_tomography_LSST_5e3cov_lmax5e4_w_binave_v1.h5"
